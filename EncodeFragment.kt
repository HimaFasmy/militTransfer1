...
class EncodeFragment : Fragment(), TextEncodingCallback, View.OnClickListener {
    private val SELECT_PIC = 100
    private var filepath: Uri? = null
    private var original_img: Bitmap? = null
    private var encoded_img: Bitmap? = null
            ...
            R.id.btn_encode ->{
                ...
                if(filepath!=null){
                    if (edittext_key.text != null){
                        ...
                        Toast.makeText(activity,"Input Key", Toast.LENGTH_SHORT).show()
                    }
                }
                else{
                    Toast.makeText(activity,"Input image", Toast.LENGTH_SHORT).show()
                }
            }
            R.id.btn_save->{
                btn_save.setOnClickListener{
                    if(encoded_img!=null && original_img!=null){
                        if (edittext_key.text!=null && edittext_message!=null ){
                            val imgtosave= encoded_img
                            ...
                            Toast.makeText(activity,"Enter the key", Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(activity,"Select image to encode", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode === SELECT_PIC && resultCode === Activity.RESULT_OK && android.R.attr.data != null) {
            filepath = data?.data
            try {
                original_img = MediaStore.Images.Media.getBitmap(activity?.contentResolver,filepath!!)
                img_encript.setImageBitmap(original_img)
            } catch (e: IOException) {
    ...
    override fun onStartTextEncoding() {
        Toast.makeText(activity,"Text Encoded", Toast.LENGTH_SHORT).show()
    }

    override fun onCompleteTextEncoding(p0: ImageSteganography?) {
        if (p0 != null && p0.isEncoded()) {
            encoded_img = p0.getEncoded_image()
            img_encript.setImageBitmap(encoded_img)
